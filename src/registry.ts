...
export class ProcessRegistry {
  ...
  getProcess(identifier: string): TroubleshootingProcess | undefined {
    ...
    for (const proc of this.processes.values()) {
      if (
        proc.processName.toLowerCase().includes(lowerIdentifier) ||
    ...
    const slugified = lowerIdentifier
      .replace(/\s+/g, "-")
      .replace(/[^a-z0-9-]/g, "");
   ...
   */
  searchProcesses(query: string): Array<{
    processId: string;
    name: string;
    description: string;
    tags: string[];
    ...
    return Array.from(this.processes.values())
      .filter((p) => {
        const searchText =
          `${p.processId} ${p.processName} ${p.description} ${p.tags.join(" ")}`.toLowerCase();
